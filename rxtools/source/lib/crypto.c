...
unsigned int arm9Decrypt(void *FIRM){ //Address of FIRM
       uint8_t* firmheader = FIRM;
       uint8_t addr = 0x4C;

       for(uint32_t i = 0; i < 4; i++)
       {
            if (firmheader[addr] == 0)
                break;
            addr += 0x30;
       }
       ...
       int arm9Size = *((unsigned int*)(FIRM + (addr + 0x8)));
       uint8_t* arm9bin = (void*)((unsigned int)FIRM + *((unsigned int*)(FIRM + addr)));

       if((arm9bin[0x80] != 0xFF && arm9bin[0x80] != 0x00) || (arm9bin[0x81] != 0xFF && arm9bin[0x81] != 0x00)) return 2; //if o3ds firm

       size_t size = arm9Size-0x800; //arm9bin encrypted info size

       int keyslot = arm9bin[0x80] == 0x00 ? 0x16 : 0x15; //keyslot changed on 9.5
        ...
        setup_aeskey(keyslot, AES_BIG_INPUT|AES_NORMAL_INPUT, arm9bin+0x10);

        uint8_t ctr[16] __attribute__((aligned(32)));
       for(uint32_t i = 0; i < 16; i++){
                ctr[i] = *(arm9bin+0x20+i);
       }

        use_aeskey(keyslot);
       for(uint32_t i = 0; i < size; i += 16) {
            set_ctr(AES_BIG_INPUT|AES_NORMAL_INPUT, ctr);
            aes_decrypt(arm9bin+0x800+i, arm9bin+0x800+i, ctr, 1, AES_CTR_MODE);
            add_ctr(ctr, 1);
       }
