...
void PadGen(){
	ConsoleInit();
	ConsoleAddText("Xorpad Generator\n");
...
u32 SdPadgen()
{
    ...
    if (FileOpen(&fp, "/movable.sed", 0)) {
        ...
        FileClose(&fp);
        if (bytesRead != 0x120) {
            ConsoleAddText("movable.sed is too small!");
            return 1;
        }
        if (memcmp(movable_seed, "SEED", 4) != 0) {
            ConsoleAddText("movable.sed is too corrupt!");
            return 1;
        }
		ConsoleAddText("movable.sed loaded!");
        setup_aeskey(0x34, AES_BIG_INPUT|AES_NORMAL_INPUT, &movable_seed[0x110]);
        use_aeskey(0x34);
    }

    if (!FileOpen(&fp, "/SDinfo.bin", 0)) {
        ConsoleAddText("Could not open SDinfo.bin!");
        return 1;
    }
    bytesRead = FileRead(&fp, info, 4, 0);

    if (!info->n_entries || info->n_entries > MAXENTRIES) {
        ConsoleAddText("Too many/few entries!");
        return 1;
    }

    ConsoleAddText("Working on SDinfo.bin ..."); ConsoleShow();
...
#define BLOCK_SIZE  (4*1024*1024)
    ...
    for (u32 i = 0; i < size_bytes; i += BLOCK_SIZE) {
		...
		sprintf(STR, "Creating Pad %i : %i%%      ", index, (i+j)/size_100);
		int x, y; ConsoleGetXY(&x, &y); y += CHAR_WIDTH * 8; x += CHAR_WIDTH*2; 
